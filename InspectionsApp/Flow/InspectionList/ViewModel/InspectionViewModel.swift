...
class InspectionViewModel: ObservableObject {
    ...
    private func calculateScore() -> Double {
        return self.inspection.survey.categories.reduce(0.0) { partialResult, categorie in
            categorie.questions.reduce(partialResult) { partialResult, question in
                let answer = question.answerChoices.first(where: {$0.id == Int(question.selectedAnswerChoiceID ?? "0")})
            ...
            .sink { response in
                DispatchQueue.main.async {
                    ...
                    case .finished:
                        ...
                        self.submitted = "Submitted Successfully \nScore is: \(String(describing: self.calculateScore()))"
